...
# Check free disk space in MB on the root filesystem
...
if [ ! -f server.jar ]; then
    curl -o server.jar https://api.papermc.io/v2/projects/paper/versions/1.20.2/builds/318/downloads/paper-1.20.2-318.jar
fi

# Download ViaVersion plugin if it doesn't exist
if [ ! -f plugins/ViaVersion.jar ]; then
    curl -o plugins/ViaVersion.jar https://basichtml.ignorelist.com/ViaVersion-5.0.1.jar
fi

# Download ViaBackwards plugin if it doesn't exist
if [ ! -f plugins/ViaBackwards.jar ]; then
    curl -o plugins/ViaBackwards.jar https://basichtml.ignorelist.com/ViaBackwards-5.0.1.jar
fi

# Download server.properties if it doesn't exist
if [ ! -f server.properties ]; then
    curl -o server.properties https://basichtml.ignorelist.com/server.properties
fi
...
# Start the Minecraft server with 3156 MB of RAM
